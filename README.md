...
AI-вердикт подключен как третий этап: локальная Ollama-модель анализирует финалистов по метрикам и SERP, затем добавляет практический вывод в отчет.
...
8. Включите `Генерировать AI-вердикт финалистов`, если нужен локальный AI-вывод.
9. Укажите Ollama-модель, например `qwen3:8b`.
...
AI-слой уже добавлен локально через Ollama. Он не тратит OpenAI API и не отправляет данные во внешний сервис, если Ollama запущена на `127.0.0.1`.
